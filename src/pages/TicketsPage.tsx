...
export default function TicketsPage() {
  ...
  const [sourceNotes, setSourceNotes] = useState<{ path: string; label: string }[]>([])
  ...
  const shouldOpenAddTicket = searchParams.get('add') === 'ticket'
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((notes) => {
        const sources = notes
          .filter((n) => n.type === 'source' || n.path.startsWith('raw/') || n.path.startsWith('docs/'))
          .sort((a, b) => (b.modified ?? 0) - (a.modified ?? 0))
          .map((n) => ({ path: n.path, label: n.path }))
        setSourceNotes(sources)
        setSelectedSource((current) => current || sources[0]?.path || '')
      })
      .catch(() => {})
  }, [])

  useEffect(() => {
    if (shouldOpenAddTicket) onOpen()
  }, [onOpen, shouldOpenAddTicket])

  const closeAddTicket = () => {
    onClose()
    if (searchParams.has('add') || searchParams.has('source')) {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-default-100 px-8 py-4">
        <div>
          ...
          <p className="text-sm text-default-400">
            Summarised follow-ups and manually added tasks — drag a card between columns to change status.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Select
            ...
            variant="bordered"
            className="w-64"
            selectedKeys={selectedSource ? new Set([selectedSource]) : new Set([])}
            onSelectionChange={(keys) => {
              const value = Array.from(keys as Set<string>)[0]
              if (value) setSelectedSource(value)
            }}
            isDisabled={sourceNotes.length === 0}
            placeholder="No source notes yet"
          >
            {sourceNotes.map((source) => (
              <SelectItem key={source.path}>{source.label}</SelectItem>
            ))}
          </Select>
          <Button
            variant="flat"
            ...
            onPress={() => setExtractPath(selectedSource)}
          >
      ...
      <NewTicketModal
        key={sourceFromQuery || 'manual'}
        isOpen={isOpen}
        onOpenChange={(open) => {
          if (open) onOpen()
          else closeAddTicket()
        }}
        initialSource={sourceFromQuery}
        onCreated={() => {
          closeAddTicket()
          refresh()
        }}
  ...
  return (
    <Modal isOpen={isOpen} onOpenChange={onOpenChange} backdrop="blur" size="lg">
      <ModalContent className="bg-content1">
        {(close) => (
          <>
            <ModalHeader>Add action item</ModalHeader>
            <ModalBody>
              ...
              <Input
                ...
                variant="bordered"
                placeholder="Optional note path"
              />
            </ModalBody>
            <ModalFooter>
