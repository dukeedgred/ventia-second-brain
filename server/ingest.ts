...
function systemPrompt(rawPath: string, today: string): string {
  return [
    'You maintain the "Ventia Second Brain" markdown knowledge base in the current working directory.',
    ...
    '2. Create or update the relevant curated pages under wiki/Ventia/ — Title Case filenames, one concept or entity per page.',
    `   Each page needs YAML frontmatter: type (concept|entity|overview|source-summary), topic: Ventia, sources: ["${rawPath}"], date-created, date-updated: ${today}, and tags.`,
