...
--      manages (standardised_asset_type_name,
...
--    → Key cols: standardised_contract_name, standardised_asset_type_name,
--                asset_id,
...
-- Run this once before the dashboard/drill-down queries below.
-- It keeps raw activity fields for audit, while the normal
-- activity_category_name/activity_name columns become standardised.
-- ============================================================
...
-- (standardised_asset_type_name level — specific enough for cross-sell)
...
--     GROUP BY source_context, standardised_contract_name, standardised_asset_type_name
-- ),

-- FOUNDATION B: What (asset_type × activity) is each contract executing?
...
-- ============================================================
-- What it shows: For every (contract, asset_type) combination,
...
-- Filter controls: Asset type picker, min peer threshold slider.
-- ============================================================
...
-- All activities ever observed on each asset type (the universe of possible scope)
UNIVERSE AS (
    SELECT DISTINCT asset_type, activity_scope
    FROM CONTRACT_ACTIVITY_SCOPE
),
-- Peer contracts doing each (asset_type × activity) — excludes the target itself
PEER_EVIDENCE AS (
    SELECT
    ...
    INNER JOIN UNIVERSE U
        ON CAP.asset_type = U.asset_type
    INNER JOIN CONTRACT_ACTIVITY_SCOPE CAS
        ON CAS.asset_type              = U.asset_type
       AND CAS.activity_scope          = U.activity_scope
    ...
    LEFT JOIN CONTRACT_ACTIVITY_SCOPE SELF
       ...
       AND SELF.asset_type                 = U.asset_type
       AND SELF.activity_scope             = U.activity_scope
...
ORDER BY opportunity_signal_score DESC, target_contract, asset_type;
...
GAPS AS (
    ...
    FROM CONTRACT_ASSET_PRESENCE CAP
    INNER JOIN UNIVERSE U ON CAP.asset_type = U.asset_type
    INNER JOIN CONTRACT_ACTIVITY_SCOPE CAS
        ON CAS.asset_type = U.asset_type
       AND CAS.activity_scope = U.activity_scope
       AND CAS.standardised_contract_name != CAP.standardised_contract_name
    ...
    GROUP BY CAP.standardised_contract_name, CAP.asset_type, U.activity_scope, CAP.asset_count
    HAVING COUNT(DISTINCT CAS.standardised_contract_name) >= 2
)
SELECT
    target_contract,
    COUNT(DISTINCT CONCAT(asset_type, '|', activity_scope)) AS total_gap_combinations,
    COUNT(DISTINCT asset_type)                                            AS asset_types_with_gaps,
    ...
    COUNT(DISTINCT CASE WHEN peers_doing_it >= 4 THEN CONCAT(asset_type, '|', activity_scope) END) AS high_confidence_gaps,
    COUNT(DISTINCT CASE WHEN peers_doing_it  = 3 THEN CONCAT(asset_type, '|', activity_scope) END) AS medium_confidence_gaps,
    COUNT(DISTINCT CASE WHEN peers_doing_it  = 2 THEN CONCAT(asset_type, '|', activity_scope) END) AS low_confidence_gaps
FROM GAPS
...
PAIRS AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN A.asset_type = B.asset_type
                            THEN A.asset_type END) AS shared_asset_types,
        COUNT(DISTINCT A.asset_type)               AS a_total_asset_types,
        COUNT(DISTINCT B.asset_type)               AS b_total_asset_types
    FROM CONTRACT_ASSET_TYPES A
    CROSS JOIN CONTRACT_ASSET_TYPES B
...
SELECT
    ...
    ROUND(
        100.0 * shared_asset_types
        / NULLIF(a_total_asset_types + b_total_asset_types - shared_asset_types, 0), 1
    )                                     AS jaccard_similarity_pct,
    -- Overlap from A's perspective: how much of A's portfolio exists in B?
    ROUND(100.0 * shared_asset_types / NULLIF(a_total_asset_types, 0), 1) AS a_coverage_in_b_pct,
    -- Overlap from B's perspective
    ROUND(100.0 * shared_asset_types / NULLIF(b_total_asset_types, 0), 1) AS b_coverage_in_a_pct
FROM PAIRS
WHERE shared_asset_types >= 3   -- at least 3 shared asset types to be meaningful
...
--   (contract × asset_type × activity). Contracts
...
-- ============================================================
-- What it shows: For a specific gap (target contract + asset type
...
-- [TARGET_ASSET_TYPE]  = the asset_type of the gap
-- [TARGET_ACTIVITY]  = the standardised_activity_scope of the gap
...
WITH GAP_PEERS AS (
    ...
    INNER JOIN vw_cross_sell_job_activity_standardised J
        ON JL.source_context = J.source_context AND JL.job_id = J.job_id
    WHERE A.standardised_asset_type_name = '[TARGET_ASSET_TYPE]'    -- e.g. 'Table Drain'
      AND J.standardised_activity_scope = '[TARGET_ACTIVITY]'    -- e.g. 'Drainage / Cleaning and Clearing'
...
TARGET_ASSETS AS (
    ...
    WHERE standardised_contract_name = '[TARGET_CONTRACT]'
      AND standardised_asset_type_name = '[TARGET_ASSET_TYPE]'
)
SELECT
    P.peer_contract,
...
INNER JOIN transport_dev.integ_transport_assets.vw_silver_transport_asset_with_category A
   ...
   AND A.standardised_asset_type_name = P.asset_type
INNER JOIN transport_dev.integ_transport_assets.vw_silver_transport_job_asset_link JL
    ON A.source_context = JL.source_context AND A.asset_id = JL.asset_id
INNER JOIN vw_cross_sell_job_activity_standardised J
    ON JL.source_context = J.source_context AND JL.job_id = J.job_id
   AND J.standardised_activity_scope = '[TARGET_ACTIVITY]'
   AND J.completed_date >= DATE_ADD(current_date(), -1095)
...
WITH PEER_RATES AS (
    ...
    WHERE A.standardised_contract_name != '[TARGET_CONTRACT]'
      AND A.standardised_asset_type_name = '[TARGET_ASSET_TYPE]'
      AND J.standardised_activity_scope = '[TARGET_ACTIVITY]'
      AND J.completed_date >= DATE_ADD(current_date(), -1095)
      AND A.standardised_contract_name IS NOT NULL
    GROUP BY A.standardised_asset_type_name, J.standardised_activity_scope, A.standardised_contract_name
),
...
SELECT
    ...
    '[TARGET_ACTIVITY]'                                                   AS activity_scope,
...
LEFT JOIN (
    ...
    INNER JOIN vw_cross_sell_job_activity_standardised J
        ON JL.source_context = J.source_context AND JL.job_id = J.job_id
    WHERE J.standardised_activity_scope = '[TARGET_ACTIVITY]'
      AND J.completed_date >= DATE_ADD(current_date(), -1095)
) JOB_CHECK
...
WITH INSP_RATE AS (
    ...
    GROUP BY A.standardised_contract_name, A.standardised_asset_type_name
),
JOB_RATE AS (
    SELECT
        A.standardised_contract_name,
...
BENCHMARK AS (
    ...
    LEFT JOIN JOB_RATE JR
       ...
       AND IR.asset_type = JR.asset_type
    GROUP BY IR.asset_type
)
SELECT
...
LEFT JOIN JOB_RATE JR
    ON IR.standardised_contract_name = JR.standardised_contract_name
   AND IR.asset_type = JR.asset_type
INNER JOIN BENCHMARK B ON IR.asset_type = B.asset_type
...
-- ============================================================
-- What it shows: Contracts managing asset types where peers
...
--   per asset type, with benchmark overlay. Filter to gaps only.
-- ============================================================
...
WITH_BENCHMARK AS (
    SELECT *,
        PERCENTILE_APPROX(cw_per_asset, 0.5)
            OVER (PARTITION BY asset_type) AS benchmark_median_cw_per_asset,
        COUNT(*) OVER (PARTITION BY asset_type) AS contracts_in_asset_type
    FROM CW_BY_CONTRACT
)
SELECT
    standardised_contract_name,
...
-- ============================================================
-- What it shows: How many distinct activity scopes does each
--   contract perform vs how many exist across all contracts for
--   their asset types? Low breadth = most room to expand.
...
WITH ALL_ACTIVITIES_PER_ASSET_TYPE AS (
    SELECT DISTINCT
        A.standardised_asset_type_name AS asset_type,
        J.standardised_activity_scope AS activity_scope
    FROM transport_dev.integ_transport_assets.vw_silver_transport_asset_with_category A
...
-- Total possible activities per contract (sum across all their asset types)
POSSIBLE_ACTIVITIES AS (
    SELECT
        A.standardised_contract_name,
        COUNT(DISTINCT CONCAT(A.asset_type, '|', U.activity_scope)) AS possible_activities
    FROM (
        SELECT DISTINCT
            standardised_contract_name,
            standardised_asset_type_name AS asset_type
    ...
    INNER JOIN ALL_ACTIVITIES_PER_ASSET_TYPE U ON A.asset_type = U.asset_type
...
ACTUAL_ACTIVITIES AS (
    SELECT
        A.standardised_contract_name,
        COUNT(DISTINCT CONCAT(A.standardised_asset_type_name, '|', J.standardised_activity_scope)) AS actual_activities,
        COUNT(DISTINCT A.standardised_asset_type_name) AS asset_types
    FROM transport_dev.integ_transport_assets.vw_silver_transport_asset_with_category A
    INNER JOIN transport_dev.integ_transport_assets.vw_silver_transport_job_asset_link JL
        ON A.source_context = JL.source_context AND A.asset_id = JL.asset_id
    INNER JOIN vw_cross_sell_job_activity_standardised J
...
WITH JOB_CLASSIFIED AS (
    SELECT
        ...
        CASE
            WHEN LOWER(J.standardised_activity_scope) LIKE '%structural%'
              OR LOWER(J.standardised_activity_scope) LIKE '%geotechnical%'
              OR LOWER(J.standardised_activity_scope) LIKE '%electrical%'
              OR LOWER(J.standardised_activity_scope) LIKE '%its%'
              OR LOWER(J.standardised_activity_scope) LIKE '%environment%'
              OR LOWER(J.standardised_activity_scope) LIKE '%pavement%'
              OR LOWER(J.standardised_activity_scope) LIKE '%rehabilitation%'
              OR LOWER(J.standardised_activity_scope) LIKE '%capital%'
              OR LOWER(J.standardised_activity_scope) LIKE '%renewal%'
            THEN 'Specialist'
            ELSE 'Routine'
        END AS job_class
    FROM transport_dev.integ_transport_assets.vw_silver_transport_asset_with_category A
...
SELECT
    ...
    COUNT(DISTINCT activity_scope)                                                AS distinct_activity_scopes,
    COUNT(DISTINCT CASE WHEN job_class = 'Specialist'
                        THEN activity_scope END)                                  AS distinct_specialist_scopes
FROM JOB_CLASSIFIED
...
--   asset types + activities that Ventia delivers for one client
...
LEFT JOIN CLIENT_ACTIVITY_SCOPE CAS_TARGET
   ...
   AND CAS_TARGET.asset_type = CAP_TARGET.asset_type
   AND CAS_TARGET.activity_scope = CAS_PEER.activity_scope
WHERE CAS_TARGET.client IS NULL
ORDER BY target_client, asset_type, activity_scope;
