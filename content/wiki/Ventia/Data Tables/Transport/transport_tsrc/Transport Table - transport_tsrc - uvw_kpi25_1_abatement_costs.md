...
table-name: uvw_kpi25_1_abatement_costs
full-name: transport_dev.transport_tsrc.uvw_kpi25_1_abatement_costs
date-created: 2026-06-09
date-updated: 2026-06-09
tags: [transport, data-table, databricks, transport-tsrc]
---

# Transport Table - transport_tsrc - uvw_kpi25_1_abatement_costs
...
| Refresh/freshness | Created: 2024-10-24T05:09:54.972Z; Last altered: 2024-10-24T05:09:54.972Z |
...
WITH step3 as (
    WITH step2 AS (
        WITH step1 AS (
            SELECT  
                ...
                , ReqInspId
                , AssessmentDate
                , concat(code,`AV_Inspection_Name`, EarliestStartDate, DueDate) as InspKey
            FROM transport_dev.transport_tsrc.utbl_asset_perf_maint_kpi25_1
        ...
        , ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`) AS KPI_BD_Counter,
        --- Set which days triggers an abatement - 5 buisness day rectification period
        CASE WHEN ROUND(
                    ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)/5,0) =
                    ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)/5
             THEN 1 
             ELSE 0
        END AS KPI_5BD_Trigger,
        --- Set which days triggers an abatement - 5 buisness day rectification period - Ceiling version
        CASE WHEN ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`) = 1 THEN 1
             WHEN mod((ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)),5) = 1 THEN 1 
             ELSE 0
        END AS KPI_5BD_Trigger_Ceiling,
        --- Set which days triggers an abatement - 10 buisness day rectification period
        CASE WHEN ROUND(
                    ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)/10,0) =
                    ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)/10
             THEN 1 
             ELSE 0
        END AS KPI_10BD_Trigger,
        --- Set which days triggers an abatement - 10 buisness day rectification period - Ceiling version
        CASE WHEN ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`) = 1 THEN 1
             WHEN mod((ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)),10) = 1 THEN 1 
             ELSE 0
        END AS KPI_10BD_Trigger_Ceiling,
        --- Set which days triggers an abatement - 20 buisness day rectification period
        CASE WHEN ROUND(
                    ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)/20,0) =
                    ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)/20
             THEN 1 
             ELSE 0
        END AS KPI_20BD_Trigger,
        --- Set which days triggers an abatement - 20 buisness day rectification period - Ceiling version
        CASE WHEN ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`) = 1 THEN 1
             WHEN mod((ROW_NUMBER() OVER(PARTITION BY a.ReqInspId ORDER BY b.`Date`)),20) = 1 THEN 1 
             ELSE 0
        END AS KPI_20BD_Trigger_Ceiling,
        --- For tracking start and end events
        CASE WHEN b.`Date` = TO_DATE(a.`AssessmentDate`) THEN 'Event End' 
    ...
    SELECT 
      ...
      --- Ongoing Abatement Cost - triggered at start of rectification period
