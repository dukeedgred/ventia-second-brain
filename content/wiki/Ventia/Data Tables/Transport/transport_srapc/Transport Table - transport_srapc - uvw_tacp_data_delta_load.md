...
table-name: uvw_tacp_data_delta_load
full-name: transport_dev.transport_srapc.uvw_tacp_data_delta_load
date-created: 2026-06-09
date-updated: 2026-06-09
tags: [transport, data-table, databricks, transport-srapc]
---

# Transport Table - transport_srapc - uvw_tacp_data_delta_load
...
| Refresh/freshness | Created: 2024-12-03T01:07:17.396Z; Last altered: 2025-07-17T01:10:32.104Z |
...
(
    select 
        ...
        , concat('AU-SRA-', b.sapName) as serviceProviderParentAssetId
        ...
        , (case a.GeomType
                    when '' then ''
                    when 'POINT' then split_part( a.wktvalue, ' ', 2)
                    else split_part( 
                                split_part( a.wktvalue, ',', 1)
                                , ' ', 2)
                end) as startLatitude
        , (case a.GeomType
                    when '' then ''
                    when 'POINT' then split_part( a.wktvalue, ' ', 1)
                    else split_part( 
                                split_part( a.wktvalue, ',', 1)
                                , ' ', 1)
                    end) as startLongitude
        , (case a.GeomType
                    when '' then ''
                        when 'POINT' then split_part( a.wktvalue, ' ', 2)
                        else split_part( 
        ...
        , null as assetGeometry -- a.wkt SAP doesn't support geoJSON
        , null as designLife
    ...
    and a.stage = 'Active'
    and month( from_utc_timestamp(a.ModifiedUTC, 'Australia/Sydney')) = month( current_date())  -- only list out records changed within the month for delta load
    and year( from_utc_timestamp(a.ModifiedUTC, 'Australia/Sydney')) = year( now())  
...
| `startLatitude` | `string` | Yes |  |
| `startLongitude` | `string` | Yes |  |
| `endLatitude` | `string` | Yes |  |
| `endLongitude` | `string` | Yes |  |
