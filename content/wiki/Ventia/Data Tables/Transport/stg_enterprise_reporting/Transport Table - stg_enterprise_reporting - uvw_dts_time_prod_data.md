...
table-name: uvw_dts_time_prod_data
full-name: transport_dev.stg_enterprise_reporting.uvw_dts_time_prod_data
date-created: 2026-06-09
date-updated: 2026-06-09
tags: [transport, data-table, databricks, stg-enterprise-reporting]
---

# Transport Table - stg_enterprise_reporting - uvw_dts_time_prod_data
...
| Refresh/freshness | Created: 2025-05-23T08:11:34.343Z; Last altered: 2025-05-23T08:11:34.343Z |
...
SELECT
  ...
  s.PST_PSA_DESC,
  s.EMPLOYEE_SUBGROUP,
  s.POSTING_STATUS_REASON,
  s.SHORTTEXT,
  s.ISRETRO,
  s.ISONBEHALF,
  CASE
    WHEN CONCAT(s.CUID_ID, s.TIMESHEET_DATE, s.VERSION, s.MODIFIEDAT) = Max_Version_Modified.LatestLookUp THEN True
    ELSE False
  END AS LatestRecord
FROM
  staged.stg_hana_cf_timesheet_db.timesheetservice_timedetailsview s
    LEFT JOIN (
      SELECT
        CONCAT(s.CUID_ID, s.TIMESHEET_DATE, s.VERSION, MAX(s.MODIFIEDAT)) as LatestLookUp
      FROM
        staged.stg_hana_cf_timesheet_db.timesheetservice_timedetailsview s
      INNER JOIN (
        SELECT
          CONCAT(t.CUID_ID, t.TIMESHEET_DATE, MAX(t.VERSION)) as LookUp
        FROM
          staged.stg_hana_cf_timesheet_db.timesheetservice_timedetailsview t
        WHERE
          t.deleted_in_source = 0
        GROUP BY
          t.CUID_ID, t.TIMESHEET_DATE
      ) AS Max_Version
      ON CONCAT(s.CUID_ID, s.TIMESHEET_DATE, s.VERSION) = Max_Version.LookUp
      WHERE
        s.deleted_in_source = 0
      GROUP BY
        s.CUID_ID, s.TIMESHEET_DATE, s.VERSION
    ) AS Max_Version_Modified
    ON CONCAT(s.CUID_ID, s.TIMESHEET_DATE, s.VERSION, s.MODIFIEDAT) = Max_Version_Modified.LatestLookUp
WHERE
  s.deleted_in_source = 0
GROUP BY
  s.CREATEDAT,
  ...
  s.ISRETRO,
  s.ISONBEHALF,
  Max_Version_Modified.LatestLookUp
```
